# ChatGPT release-review packet

## Instructions

# Prompt for optional Sol High release review

Review the attached release packet as a read-only principal engineer. Inspect the current plan, source, diff, test summary, game constraints, and live rollback policy.

Return `APPROVED` or `CHANGES_REQUIRED` first. Block only for:

- illegal action or escaping-exception risk;
- CPU/TLE or unbounded-work risk;
- stale game mechanics;
- economy/logistics/state-machine deadlock;
- invalid evaluation evidence or material map/side overfit;
- packaging, activation, durable-state, or rollback defects.

For each blocking issue provide file/symbol, consequence, evidence, required correction, and exact retest. Keep the entire review under 700 words.
